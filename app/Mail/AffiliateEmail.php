...
class AffiliateEmail extends Mailable
{
    ...
    $ghs_exchange_rate = DB::selectOne('SELECT value FROM settings WHERE settings_key = ? LIMIT 1', ['usd_to_ghs']);
        ...
        return $this->view('affiliate_sale_email', ['message' => $message])
                    ->with(['affiliate_email'=> $this->affiliate_email,'firstName'=>$this->firstName,'productPrice'=>strval(intval($this->productPrice)/intval($naira_exchange_rate->value )),'commission'=>strval(inval($this->commission)/intval($naira_exchange_rate->value )), "customer_name"=>$this->customer_name, "productName"=>$this->productName])
